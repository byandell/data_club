...
Here is the SD line plotted over the scatterplot of data. A red circle is added at the mean of `weight` and `hindfoot_length`, and a rectangle extending out 2 SDs in each direction is shown in blue. The data points are made a bit smaller and grayer to deempathize them.
...
The introductory book [_Statisticss_ by Freedman, Pisani and Purvis]() has a nice intrepretation, which is partially presented at [this website](http://www.analytictech.com/mb313/correlat.htm). Philip Stark's [Regression](http://www.stat.berkeley.edu/~stark/SticiGui/Text/regression.htm) chapter has a nice interactive tool where you can vary the sample size and correlation to develop some intuition. 

If your scatterplot (or the blue rectangle defined by SDs) is roughly square, the correlation is roughly

```
cor = 1 - width / length
```

with `length` being the length along the green line`width` being the distance out perpendicular to the green line of the blob of data.
...
```{r}
surveys_complete %>% summarize_at(vars(hindfoot_length,weight), sd)
```
